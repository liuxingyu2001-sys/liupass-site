...
- ✅ Reward types: COMMAND / ITEM / CE (CraftEngine model items) / IE (ItemEdit custom items) / MONEY / POINTS, all referenced by reward ID
...
| CraftEngine | optional | GUI icon models / CE rewards |
| ItemEdit | optional | IE custom item rewards (skipped with a warning when missing) |
...
> Reward types: `COMMAND` / `ITEM` / `CE` (grants CE item directly; skipped with a warning when CE is missing) / `IE` (grants ItemEdit item directly; skipped with a warning when IE is missing) / `MONEY` / `POINTS`
...
> Level-cell icon priority: `icon-model` → CE `model` / IE `model` → ITEM `material`
