...
- ✅ 奖励类型：COMMAND / ITEM / CE（CraftEngine 模型物品）/ IE（ItemEdit 自定义物品）/ MONEY / POINTS，全部通过奖励 ID 引用复用
...
| CraftEngine | 可选 | GUI 图标模型显示 / CE 奖励 |
| ItemEdit | 可选 | IE 自定义物品奖励（缺失时跳过并警告） |
...
> 奖励类型：`COMMAND` / `ITEM` / `CE`（直接发放 CE 物品，未装 CE 时跳过并警告）/ `IE`（直接发放 ItemEdit 物品，未装 IE 时跳过并警告）/ `MONEY` / `POINTS`
...
> 等级格子图标优先级：`icon-model` → CE 的 `model` / IE 的 `model` → ITEM 的 `material`
